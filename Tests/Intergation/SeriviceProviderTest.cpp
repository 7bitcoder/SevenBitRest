...
class SeriviceProviderTest : public ::testing::Test
{
  protected:
    ...
    sd::ServiceContainer singletons;
    sd::ServiceProvider provider{collection, singletons};
...
TEST_F(SeriviceProviderTest, CircularDependency)
{
    ...
    EXPECT_THROW(provider.getService<CircularDependencyA>(), sd::CircularDependencyException);
}
...
TEST_F(SeriviceProviderTest, GetRequiredService)
{
    ...
    EXPECT_THROW(provider.getRequiredService<TransientRefClass>(), sd::ServiceNotRegisteredException);
}
...
TEST_F(SeriviceProviderTest, NotTransientCreationForbidden)
{
    ...
    EXPECT_THROW(provider.createService<SimpleClass>(), sd::NotTransientException);
    EXPECT_THROW(provider.createService<OneRefClass>(), sd::NotTransientException);
}
...
TEST_F(SeriviceProviderTest, TransientGetServiceForbidden)
{
    ...
    EXPECT_NO_THROW(provider.createService<TransientClass>());
    EXPECT_TRUE(provider.createService<TransientClass>());
    EXPECT_THROW(provider.getService<TransientClass>(), sd::TransientForbidException);
}
...
{
    auto self = provider.getService<sd::ServiceProvider>();
    EXPECT_TRUE(self);
...
{
    auto &self = provider.getRequiredService<sd::ServiceProvider>();
    EXPECT_EQ(&self, &provider);
}
...
TEST_F(SeriviceProviderTest, GetAllInheritedServicesForbidTransient)
{
    ...
    EXPECT_THROW(provider.getServices<TransientClass>(), sd::TransientForbidException);
}
...
TEST_F(SeriviceProviderTest, PrebuildSingeletons)
{
    ...
    provider.getService<IInheranceClass>();
    EXPECT_NO_THROW(provider.prebuildSingeletons());
}
