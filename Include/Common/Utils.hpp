...
namespace sd::utils
{
    ...
    CliVariables EXPORT getCliVariables(int argc, char **argv);

    EnvVariables EXPORT getEnvVariables(std::string prefix);

    std::string EXPORT demangle(const std::string &name);

    inline void updateJsonWithValue(Json::object_t &root, const std::string &key, const std::string &value)
    {
        auto keySegments = utils::split(key, ':');
        Json::object_t *current = &root;
        if (keySegments.size() < 1)
        {
            throw std::runtime_error("bad configuration parameter");
        }
        for (int i = 0; i < keySegments.size(); i++)
        {
            auto keySegment = std::string{keySegments[i]};
            auto &ob = (*current)[keySegment];

            if (i == keySegments.size() - 1)
            {
                ob = value;
                break;
            }
            if (ob.is_uninitialized())
            {
                ob = tao::json::empty_object;
            }
            if (!ob.is_object())
            {
                throw std::runtime_error("Bad configuration key: " + keySegment + "' is not an object");
            }
            current = &(ob.get_object());
        }
    }
} // namespace sd::utils